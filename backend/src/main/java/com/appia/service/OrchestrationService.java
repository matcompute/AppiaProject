...
public class OrchestrationService {
    ...
    public Optional<NetworkNode> findBestNode(ServiceFunctionChain sfc) {
        ...
        // Normalisation ranges across candidates
        ...
        return candidates.stream().min(Comparator.comparingDouble(n -> {
            double normCarbon  = maxCarbon  > 0 ? n.getCarbonIntensityGco2Kwh() / maxCarbon  : 0;
            double normLatency = maxLatency > 0 ? n.getProcessingLatencyMs()    / maxLatency : 0;
            double normCost    = maxCost    > 0 ? n.getEnergyCostEurKwh()       / maxCost    : 0;
            double normLoad    = n.getCpuLoadPct() / 100.0;
            return wCarbon * normCarbon + wLatency * normLatency + wCost * normCost + wLoad * normLoad;
        }));
    }
    ...
    public String getAiRecommendations() {
        return geminiAdvisor.getProactiveRecommendations(nodeRepo.findAll(), sfcRepo.findAll());
    }

    public String analyzeScenario(String question) {
        return geminiAdvisor.analyzeScenario(question, nodeRepo.findAll(), sfcRepo.findAll());
    }

    // ── Analytics ─────────────────────────────────────────────────────────────

    public List<PlacementRecord> getRecentPlacements(int limit) {
        return placementRepo.findRecentPlacements(PageRequest.of(0, limit));
    }

    public Long getCriticalViolationCount() {
        return placementRepo.countCriticalViolations();
    }

    public Double getAvgCarbonWhenSlaWasMet() {
        return placementRepo.avgCarbonWhenSlaWasMet();
    }
}
